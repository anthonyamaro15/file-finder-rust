...
pub fn run_simple_performance_test() {
    ...
    // Calculate overhead
    let highlighting_overhead =
        highlighting_time.as_micros() as f64 - baseline_time.as_micros() as f64;
    ...
    println!(
        ...
        highlighting_overhead, overhead_percentage
    );
    println!(
        "  Per-file highlighting cost:       {:.3}μs",
        highlighting_overhead / large_dataset.len() as f64
    );
    println!(
        "  Per-file full UI cost:            {:.3}μs",
        full_ui_time.as_micros() as f64 / large_dataset.len() as f64
    );
...
fn test_memory_impact() {
    ...
    println!(
        ...
        std::mem::size_of_val(&simple) + simple.iter().map(|s| s.len()).sum::<usize>()
    );
    println!(
        "  Highlighted spans: ~{} bytes (estimated)",
        std::mem::size_of_val(&highlighted) + highlighted.len() * 200
    ); // Rough estimate
...
fn test_realistic_scenario() {
    ...
    let start = Instant::now();
    for _ in 0..1000 {
        // Simulate 1000 keystrokes/renders
