...
fn generate_test_files(count: usize) -> Vec<String> {
    let prefixes = [
        "src", "lib", "bin", "test", "doc", "config", "build", "target",
    ];
    let extensions = ["rs", "toml", "md", "txt", "json", "yaml", "lock"];
    let names = [
        "main", "lib", "mod", "utils", "helper", "core", "api", "client",
    ];
...
pub fn analyze_memory_usage() {
    ...
    println!(
        "  Simple items:      ~{} KB",
        estimate_listitem_memory(&simple_items)
    );
    println!(
        "  Highlighted items: ~{} KB",
        estimate_listitem_memory(&highlighted_items)
    );
