...
pub use settings::{Settings, Configuration};
...
pub fn ensure_config_directories() -> AppResult<()> {
    ...
    if !config_dir.exists() {
        fs::create_dir_all(&config_dir)
            .map_err(|e| AppError::Configuration {
                message: format!("Failed to create config directory '{}': {}", config_dir.display(), e)
            })?;
    }
    ...
    if !themes_dir.exists() {
        fs::create_dir_all(&themes_dir)
            .map_err(|e| AppError::Configuration {
                message: format!("Failed to create themes directory '{}': {}", themes_dir.display(), e)
            })?;
    }
...
pub fn reset_configuration() -> AppResult<()> {
    ...
    if config_dir.exists() {
        fs::remove_dir_all(&config_dir)
            .map_err(|e| AppError::Configuration {
                message: format!("Failed to remove config directory '{}': {}", config_dir.display(), e)
            })?;
    }
