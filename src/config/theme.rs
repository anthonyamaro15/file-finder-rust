...
impl Theme {
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> AppResult<Self> {
        let content = fs::read_to_string(&path)
            .map_err(|e| AppError::Configuration {
                message: format!("Failed to read theme file '{}': {}", path.as_ref().display(), e)
            })?;

        let theme: Self = toml::from_str(&content)
            .map_err(|e| AppError::Configuration {
                message: format!("Failed to parse TOML theme file '{}': {}", path.as_ref().display(), e)
            })?;
    ...
    pub fn save_to_file<P: AsRef<Path>>(&self, path: P) -> AppResult<()> {
        ...
        fs::write(&path, toml_content)
            .map_err(|e| AppError::Configuration {
                message: format!("Failed to write theme to file '{}': {}", path.as_ref().display(), e)
            })?;
    ...
    fn parse_themed_color(theme_color: &ThemeColor) -> AppResult<Style> {
        let modifiers = theme_color.modifiers.as_ref()
