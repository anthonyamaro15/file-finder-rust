...
pub fn highlight_search_term(text: &str, search_term: &str, normal_style: Style, highlight_style: Style) -> Line<'static> {
...
/// Optimized version for batch processing of search results
pub fn highlight_search_results(
    ...
    if search_term.is_empty() {
        return items.iter()
            .map(|item| Line::from(Span::styled(item.clone(), normal_style)))
            .collect();
    }
    
    items.iter()
        .map(|item| highlight_search_term(item, search_term, normal_style, highlight_style))
        .collect()
}
